...
tableEl.addEventListener("click", (e) => {
  ...
  if (
    ...
    const response = confirm(`Are you sure you want to delete ${studentName}`);

    if (response) {
      // User confirms to delete
      console.log("Deleted");
      e.target.closest("tr").remove();

      // to adjust ids, get all rows and iterate over
      const tableRowsNodeList = tableEl.querySelectorAll("tbody tr");
      // Make it an array so we can use iterator methods:
      const tableRows = Array.from(tableRowsNodeList);
      // Change the ids of the rows
      tableRows.forEach((row, index) => {
        row.children[0].innerText = index + 1;
      });
    } else {
      // User rejects to delete
